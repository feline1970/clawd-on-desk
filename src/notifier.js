...
function classifyTerminalResult(agentId, event, state) {
...
function formatSourceLabel(agentId) {
  if (!agentId) return "unknown";
  if (agentId === "cursor-agent") return "cursor";
  return agentId;
}
...
function buildNotificationText(payload, result, tsNow) {
  ...
  const lines = [
    ...
    `来源：${formatSourceLabel(payload.agentId)}`,
  ...
  return {
    title,
    message: lines.join("\n"),
    dedupeKey: `${payload.agentId || "unknown"}:${safeSessionId}:${result}:${payload.event || "unknown"}`,
  };
}
...
function createPushoverNotifier(options = {}) {
  ...
  async function sendCursorTerminalNotification(payload = {}) {
    const result = classifyTerminalResult(payload.agentId, payload.event, payload.state);
    ...
    try {
      ...
      if (!res.ok) return { skipped: "http-error", status: res.status };
      return { sent: true };
    } catch (err) {
